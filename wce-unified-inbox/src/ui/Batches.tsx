...
export function Batches({ initialListId }: { initialListId?: { id: string; nonce: number } | null } = {}) {
  ...
  // True while we've asked the engine for fresh wording and are polling for it.
  ...
  async function loadVariants(v: string): Promise<{ id: string; body: string }[]> {
  ...
  useEffect(() => {
    ...
    void loadVariants(v).then(setInviteVariants)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [batchVenue])

  // "Refresh wording": retire this venue's current options so the engine writes a
  // brand-new set (a fresh Anthropic pass), then poll until they land. The panel
  // only offers this when options already exist — i.e. the venue is scheduled, so
  // the engine will regenerate. Prompt once the engine is on the faster cadence.
  async function refreshVariants() {
    const v = batchVenue.trim()
    if (!v || rewording) return
    setRewording(true)
    setInviteVariants([])
    const q = supabase as unknown as {
      from: (t: string) => { update: (val: unknown) => { ilike: (c: string, p: string) => { eq: (c2: string, v2: boolean) => Promise<unknown> } } }
    }
    await q.from('inbox_invite_variants').update({ active: false }).ilike('venue', `%${v}%`).eq('active', true)
    const started = Date.now()
    const tick = async () => {
      const rows = await loadVariants(v)
      if (rows.length >= 3) { setInviteVariants(rows); setRewording(false); return }
      if (Date.now() - started > 150_000) { setRewording(false); return } // give up after ~2.5m
  ...
  // Discard the open preview. Every preview (fresh build OR an opened scheduler
  // draft) is backed by a real inbox_batches row, so clearing local state alone
  // left the draft in the DB — which is why a discarded scheduler draft kept
  // reappearing in the "New batch" awaiting-approval menu. Cancel the row (and
  // untick its items) so it drops out for good; the recurring schedule that
  // generated it is untouched, so next week still builds a fresh one.
  async function discard() {
    const id = batchId
    resetAll()
    if (!id) return
    await supabase.from('inbox_batches').update({ status: 'canceled' }).eq('id', id)
    await supabase.from('inbox_batch_items').update({ status: 'skipped' }).eq('batch_id', id).eq('status', 'pending')
    loadScheduledDrafts() // re-fetch so the menu reflects the removal immediately
  }
