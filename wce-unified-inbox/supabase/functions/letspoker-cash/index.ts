...
//                  rosters, so without these nothing ever auto-opened. Tables
//                  open PUBLIC (isPublic:true on AddTable, confirmed with a
//                  ModifyTable) so players can see and join them.
...
// isPublic:true so copied tables open public too (past logs don't carry it).
function specFromTable(t: TableInfo): any {
  const spec: any = { tableSize: t.tableSize, isPublic: true };
...
// Tables open PUBLIC: isPublic:true on the AddTable eventData plus a follow-up
// ModifyTable{isPublic:true} to confirm (some LP builds ignore it on create).
...
async function runOpen(cookie: string, sgid: string, clubId: string, opts: { planId?: string; date?: string; dryRun: boolean; start?: boolean }): Promise<Response> {
  ...
  for (const p of plans) {
    ...
    for (const spec of specs) {
      // Open the table PUBLIC: isPublic on create, then a ModifyTable confirm
      // (some LP builds ignore isPublic on AddTable and default to private).
      const c = await createLogItem(cookie, sgid, clubId, eventId, { eventType: "AddTable", eventData: { isPublic: true, ...spec } });
      await logCash({ source: "open", op: "open", ref: p.id, http_status: c.status, ok: c.ok, detail: `AddTable${derivedFrom ? ` (copied from ${derivedFrom})` : ""} ${JSON.stringify({ isPublic: true, ...spec }).slice(0, 180)} -> ${c.ok ? c.id : c.error}` });
      if (c.ok && c.id) {
        const pub = await createLogItem(cookie, sgid, clubId, eventId, { eventType: "ModifyTable", parentLogId: c.id, eventData: { isPublic: true } });
        await logCash({ source: "open", op: "public", ref: p.id, http_status: pub.status, ok: pub.ok, detail: `ModifyTable{isPublic:true} table=${c.id} -> ${pub.ok ? "OK" : pub.error}` });
      }
