...
- **Home financials** (`0056`, g41): `extractFinancials()` harvests label:value money lines from every TD sheet's financial tabs (Overall Financials / Reconciliation / Club Financials / WCP Invoice) into `inbox_game_financials` — ALL sheets incl. the `--all` backfill, read-only. Home shows a Financials card: this-week tiles (net profit actual / calc, gross buy-ins, cash rake) + weekly net-profit bars (blue profit / red loss, hover tooltip, table view). **The metric mapping is the `inbox_financial_summary` view (crude keyword match) — once real labels land, inspect `select distinct norm_label from inbox_game_financials` and refine the view live; no code change needed.**
- **LP banner channel** (g40): the Social composer has an `📱 LP banner` chip — a publishing post's artwork (direct image URL) is fetched, data-URL'd and pushed through the `letspoker-cover` Supabase edge function (admin session cookie kept fresh by the `letspoker-login` cron + server-side TOTP), which uploads it to the club library and activates it as the live LetsPoker app club cover. Two-leg publish: a succeeded leg is never re-run; one failed leg = status `posted` with a `partial:` warning (protects against double social posts). LP-banner-only posts skip Postiz entirely. Event-level banners not mirrored yet (only the club cover mutation is known).
- **Game-week social autopilot** (`0055`, `sync/socialauto.ts`, g38): every active game schedule gets two AI-drafted promo posts per occurrence (day-before 17:30 + morning-of 09:30, 7-day lookahead, one Anthropic call per occurrence) saved as DRAFTS on the Social calendar with an `auto` chip. Approval-gated: "Schedule all drafts" approves the week in one tap. Idempotent via `social_posts.source_key` (`promo:<schedule_id>:<game_date>:<slot>`) — deleting a draft does NOT resurrect it. Publisher change: the CAPTION is what gets posted; title is just the calendar label. Postiz publisher targets real integration IDs (fetched + matched against platform chips at publish time; empty chips = all channels), uploads assets via /upload (direct-image URLs only; a failure posts text-only with the link appended).
- **Attendance scoring** (`0054`, applied live): `inbox_attendance_stats` view — per player-norm distinct game NIGHTS all-time (tourney/cash split, last-70d count, first/last seen, venues array) over LP entries + TD attendees. LP tournament data is LIVE already via the `letspoker-*` pg_cron fleet (harvest nightly 18:00 UTC, calendar every 4h — 857 events / 20k entries, May 2024→today); the TD side goes all-time after the `--all` backfill and stays current via the live sync. Players tab: **venue filter** (played there per attendance, or CRM-tagged), **sort by Most games / Recently seen** with #rank badges, and a 🎟 games chip (`34 · 28T/6C · last 12 Jun`) on every card.
